...
<x-layouts.app :tenant="$tenant" :active="$active" title="LibrairePro · Journal d'activité">
    <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="min-w-0">
                <p class="text-sm font-semibold text-brand">Profil · Traçabilité</p>
                <h1 class="mt-1 text-2xl font-semibold tracking-normal text-slate-950 dark:text-white">Journal d’activité</h1>
                <p class="mt-2 max-w-3xl text-sm text-slate-600 dark:text-slate-300">Vue propriétaire des actions enregistrées dans l’application: utilisateur, date, module, IP et données utiles nettoyées.</p>
            </div>
            <div class="flex flex-col gap-2 sm:flex-row">
                <a href="{{ route('profile') }}" class="inline-flex h-11 items-center justify-center rounded-lg border border-slate-200 px-4 text-sm font-semibold text-slate-700 transition hover:border-brand/40 hover:text-brand dark:border-white/10 dark:text-slate-200">Retour profil</a>
                <a href="{{ route('module', ['module' => 'settings', 'section' => 'users']) }}" class="inline-flex h-11 items-center justify-center rounded-lg bg-brand px-4 text-sm font-semibold text-white shadow-sm shadow-indigo-500/20 transition hover:brightness-110">Utilisateurs</a>
            </div>
        </div>

        <div class="mt-5 grid gap-3 md:grid-cols-3">
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                <p class="text-xs font-semibold uppercase text-slate-500">Total enregistré</p>
                <strong class="mt-2 block text-2xl font-black text-slate-950 dark:text-white">{{ number_format($totals['all'], 0, ',', ' ') }}</strong>
            </div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                <p class="text-xs font-semibold uppercase text-slate-500">Aujourd’hui</p>
                <strong class="mt-2 block text-2xl font-black text-slate-950 dark:text-white">{{ number_format($totals['today'], 0, ',', ' ') }}</strong>
            </div>
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                <p class="text-xs font-semibold uppercase text-slate-500">Utilisateurs tracés</p>
    ...
    <section class="mt-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
        <form action="{{ route('profile.activity') }}" method="GET" class="grid gap-3 xl:grid-cols-[minmax(180px,1fr)_150px_150px_130px_minmax(220px,1.3fr)_auto_auto] xl:items-end">
            <label class="space-y-1.5">
                <span class="text-xs font-semibold uppercase text-slate-500">Utilisateur</span>
            ...
            <label class="space-y-1.5">
                <span class="text-xs font-semibold uppercase text-slate-500">Du</span>
            ...
            <label class="space-y-1.5">
                <span class="text-xs font-semibold uppercase text-slate-500">Au</span>
            ...
            <label class="space-y-1.5">
                <span class="text-xs font-semibold uppercase text-slate-500">Méthode</span>
            ...
            <label class="space-y-1.5">
                <span class="text-xs font-semibold uppercase text-slate-500">Recherche</span>
                <input name="q" value="{{ $filters['q'] }}" placeholder="Route, écran, action..." class="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-950 dark:border-white/10 dark:bg-slate-900 dark:text-white">
            </label>
            <button class="h-11 rounded-lg bg-brand px-5 text-sm font-semibold text-white shadow-sm shadow-indigo-500/20 transition hover:brightness-110">Filtrer</button>
            <a href="{{ route('profile.activity') }}" class="inline-flex h-11 items-center justify-center rounded-lg border border-slate-200 px-5 text-sm font-semibold text-slate-700 transition hover:border-brand/40 hover:text-brand dark:border-white/10 dark:text-slate-200">Effacer</a>
        </form>
    </section>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1120px] text-left text-sm">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-white/10">
                        ...
                        <tr class="transition hover:bg-slate-50 dark:hover:bg-white/5">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <span class="grid size-9 shrink-0 place-items-center rounded-lg bg-brand/10 text-xs font-bold text-brand">{{ Str::upper(Str::substr($log->user?->name ?? 'Système', 0, 2)) }}</span>
                                    <div class="min-w-0">
                                        <p class="truncate font-semibold text-slate-950 dark:text-white">{{ $log->user?->name ?? 'Système / invité' }}</p>
                                        <p class="truncate text-xs text-slate-500">{{ $log->user?->email ?? 'Action système' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <p class="max-w-[280px] truncate font-semibold text-slate-950 dark:text-white">{{ $log->action }}</p>
                            ...
                            <td class="px-4 py-3 text-right">
                                <button type="button" onclick="document.getElementById('audit-log-{{ $log->id }}').showModal()" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold transition hover:border-brand hover:text-brand dark:border-white/10">Voir détail</button>
                            </td>
                        </tr>
                        <dialog id="audit-log-{{ $log->id }}" class="app-dialog w-[min(920px,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-0 text-slate-950 shadow-2xl backdrop:bg-slate-950/45 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100">
                            <div class="border-b border-slate-200 p-5 dark:border-white/10">
                                <div class="flex items-start justify-between gap-4">
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-brand">Détail journal</p>
                                        <h3 class="mt-1 truncate text-xl font-semibold">{{ $log->action }}</h3>
                                        <p class="mt-1 text-sm text-slate-500">{{ $log->created_at?->format('d/m/Y H:i:s') }} · {{ $log->user?->name ?? 'Système / invité' }}</p>
                                    </div>
                            ...
                            <div class="grid max-h-[75vh] gap-4 overflow-y-auto p-5 lg:grid-cols-[1fr_1.2fr]">
                                <div class="space-y-3">
                                    <div class="rounded-xl bg-slate-50 p-4 dark:bg-white/5">
                                        <p class="text-xs font-semibold uppercase text-slate-500">Requête</p>
                                        <dl class="mt-3 space-y-2 text-sm">
                                            <div class="flex justify-between gap-3"><dt class="text-slate-500">Méthode</dt><dd class="font-semibold">{{ $method }}</dd></div>
                                            <div class="flex justify-between gap-3"><dt class="text-slate-500">Route</dt><dd class="max-w-[220px] truncate font-semibold">{{ data_get($properties, 'route', '—') }}</dd></div>
                                            <div class="flex justify-between gap-3"><dt class="text-slate-500">Statut</dt><dd class="font-semibold">{{ data_get($properties, 'status_code', '—') }}</dd></div>
                                            <div class="flex justify-between gap-3"><dt class="text-slate-500">IP</dt><dd class="font-semibold">{{ data_get($properties, 'ip', '—') }}</dd></div>
                                        </dl>
                                    </div>
                                    <div class="rounded-xl bg-slate-50 p-4 dark:bg-white/5">
                                        <p class="text-xs font-semibold uppercase text-slate-500">Sujet</p>
                                ...
                                <div class="space-y-3">
                                    <div>
                                        <p class="mb-2 text-xs font-semibold uppercase text-slate-500">Données envoyées</p>
                                    ...
                                    <div>
                                        <p class="mb-2 text-xs font-semibold uppercase text-slate-500">Paramètres route</p>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-14 text-center text-sm text-slate-500">Aucune action ne correspond aux filtres.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
