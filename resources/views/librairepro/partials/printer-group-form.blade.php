...
<div class="grid gap-5 p-5">
    ...
    <section class="rounded-xl border border-slate-200 p-4 dark:border-white/10" data-printer-group-routing>
        <div class="flex flex-col gap-1">
            <h4 class="font-semibold">Routage par catégories</h4>
            <p class="text-sm text-slate-500">Sélectionnez les familles envoyées à ce groupe. L’option “Tout le reste” reçoit les articles sans règle dédiée.</p>
        </div>
        <label class="mt-3 flex cursor-pointer items-start gap-3 rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-950 transition hover:border-amber-400 hover:bg-amber-100/70 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-100">
            <input name="catch_all" value="1" type="checkbox" @checked(old('catch_all', $hasCatchAll)) class="mt-1 size-4 shrink-0 accent-amber-600" data-printer-catch-all>
            <span class="min-w-0">
                <span class="flex flex-wrap items-center gap-2 font-bold">
                    <span class="inline-flex rounded-full bg-amber-200 px-2.5 py-0.5 text-[11px] font-black uppercase tracking-wide text-amber-900 dark:bg-amber-400/20 dark:text-amber-100">Tout le reste</span>
                    Catégories non assignées
                </span>
                <span class="mt-1 block text-xs font-medium leading-relaxed text-amber-800 dark:text-amber-100/80">
                    Activez cette règle si ce groupe doit imprimer tous les articles qui ne sont envoyés à aucun autre groupe. Vous n’avez pas besoin de sélectionner toutes les catégories une par une.
                </span>
            </span>
        </label>
        <div class="mt-3 hidden rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100" data-printer-catch-all-hint>
            Toutes les catégories ci-dessous sont couvertes automatiquement. Vous pouvez désactiver “Tout le reste” pour choisir seulement certaines catégories.
        </div>
        <div class="mt-3 grid max-h-72 gap-2 overflow-y-auto pr-1 md:grid-cols-2">
            @forelse ($categories as $category)
                <label class="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm transition dark:border-white/10 dark:bg-slate-950/40" data-printer-category-option>
                    <input name="category_ids[]" value="{{ $category->id }}" type="checkbox" @checked(in_array((int) $category->id, $selectedCategoryIds, true)) class="size-4 accent-[var(--brand-primary)]" data-printer-category-checkbox data-original-checked="{{ in_array((int) $category->id, $selectedCategoryIds, true) ? '1' : '0' }}">
