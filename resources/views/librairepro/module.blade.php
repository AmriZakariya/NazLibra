...
<x-layouts.app :tenant="$tenant" :active="$active" :title="$pageTitle">
        ...
        @elseif ($invoiceSection === 'invoice-add' || ($invoiceSection === 'invoice-edit' && $editInvoice))
            ...
            <section class="mt-6 grid min-w-0 gap-6 2xl:grid-cols-[minmax(0,1fr)_340px]" data-invoice-screen>
                <form action="{{ $invoiceFormAction }}" method="POST" class="min-w-0 space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.03]" data-invoice-form>
                    ...
                    @if ($errors->any())
                        <div class="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200" data-invoice-error-banner role="alert">
                    ...
                    <div class="grid gap-4 lg:grid-cols-4">
                        <label class="space-y-1.5 lg:col-span-2">
                            ...
                            @error('customer_id')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror
                        </label>
                        ...
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Date d'émission *</span><input name="issue_date" data-invoice-issue-date value="{{ old('issue_date', $editInvoice?->issue_date?->toDateString() ?? now()->toDateString()) }}" type="date" required class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900">@error('issue_date')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror</label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Conditions de règlement</span><select data-invoice-terms class="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm dark:border-white/10 dark:bg-slate-900"><option value="">Personnalisée</option><option value="0">À réception</option><option value="15">Net 15 jours</option><option value="30">Net 30 jours</option><option value="45">Net 45 jours</option><option value="60">Net 60 jours</option><option value="eom">Fin de mois</option></select></label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Échéance *</span><input name="due_date" data-invoice-due-date value="{{ old('due_date', $editInvoice?->due_date?->toDateString() ?? now()->addDays(15)->toDateString()) }}" type="date" required class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900">@error('due_date')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror</label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Date de prestation</span><input name="service_date" value="{{ old('service_date', $editInvoice?->service_date?->toDateString()) }}" type="date" class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900">@error('service_date')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror</label>

                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Nouveau client</span><input name="customer_name" value="{{ old('customer_name', data_get($invoiceCustomerSnapshot, 'name')) }}" class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Nom client si non sélectionné"></label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Société</span><input name="company_name" value="{{ old('company_name', data_get($invoiceCustomerSnapshot, 'company_name')) }}" class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Raison sociale"></label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">ICE</span><input name="ice" value="{{ old('ice', data_get($invoiceCustomerSnapshot, 'ice')) }}" inputmode="numeric" class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Identifiant Commun de l'Entreprise">@error('ice')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror</label>
                        ...
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Email</span><input name="email" type="email" value="{{ old('email', data_get($invoiceCustomerSnapshot, 'email')) }}" class="h-11 w-full rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="client@example.com">@error('email')<span class="block text-xs font-medium text-rose-600">{{ $message }}</span>@enderror</label>

                        <label class="space-y-1.5 lg:col-span-2"><span class="text-xs font-semibold uppercase text-slate-500">Adresse de facturation</span><textarea name="billing_address" class="min-h-16 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Adresse...">{{ old('billing_address', data_get($invoiceCustomerSnapshot, 'billing_address')) }}</textarea></label>
                        <label class="space-y-1.5 lg:col-span-2"><span class="text-xs font-semibold uppercase text-slate-500">Adresse de livraison</span><textarea name="shipping_address" class="min-h-16 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Si différente de la facturation...">{{ old('shipping_address', data_get($invoiceCustomerSnapshot, 'shipping_address')) }}</textarea></label>
                    </div>
                            ...
                            @foreach ($initialInvoiceLines as $i => $line)
                                <div class="invoice-line relative grid gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-white/10 dark:bg-slate-950/60 lg:grid-cols-[minmax(280px,1fr)_86px_118px_118px_82px_132px_40px] lg:items-start" data-line-index="{{ $i }}">
                                    <div class="invoice-item-picker" data-invoice-item-picker>
                                        ...
                                        @php $lineTtc = (bool) data_get($line, 'tax_inclusive'); @endphp
                                        <div class="mt-2 inline-flex items-center gap-1">
                                            <span class="text-[11px] font-semibold uppercase text-slate-400">Prix</span>
                                            <span class="inline-flex rounded-lg border border-slate-200 p-0.5 dark:border-white/10" role="group" aria-label="Prix hors taxe ou toutes taxes comprises">
                                                <button type="button" data-invoice-tax-mode="0" class="rounded-md px-2 py-0.5 text-[11px] font-bold transition {{ $lineTtc ? 'text-slate-500' : 'bg-brand text-white' }}">HT</button>
                                                <button type="button" data-invoice-tax-mode="1" class="rounded-md px-2 py-0.5 text-[11px] font-bold transition {{ $lineTtc ? 'bg-brand text-white' : 'text-slate-500' }}">TTC</button>
                                            </span>
                                        </div>
                                    ...
                                    <label class="block"><span class="block text-xs font-semibold uppercase text-slate-500 lg:hidden">Remise</span>
                                        <div class="mt-1 flex lg:mt-0">
                                            <input name="lines[{{ $i }}][discount_value]" data-invoice-discount-line type="number" min="0" step="0.01" value="{{ data_get($line, 'discount_value') }}" class="h-11 w-full min-w-0 rounded-l-lg border border-slate-200 px-2 text-sm text-right dark:border-white/10 dark:bg-slate-900" placeholder="0">
                                            <button type="button" data-invoice-discount-toggle class="h-11 shrink-0 rounded-r-lg border border-l-0 border-slate-200 bg-slate-50 px-2 text-xs font-bold text-slate-600 transition hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-slate-300" title="Basculer remise fixe / pourcentage">{{ data_get($line, 'discount_type', 'fixed') === 'percentage' ? '%' : 'DH' }}</button>
                                        </div>
                                    </label>
                                    ...
                                    <input type="hidden" name="lines[{{ $i }}][discount_type]" data-invoice-discount-type value="{{ data_get($line, 'discount_type', 'fixed') }}">
                    ...
                    <div class="grid gap-4 lg:grid-cols-[1fr_200px_200px]">
                        ...
                        @php $docDiscountPercentage = old('document_discount_type', $editInvoice?->document_discount_type ?? 'fixed') === 'percentage'; @endphp
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Remise globale</span>
                            <div class="flex">
                                <input name="document_discount_value" data-invoice-discount value="{{ old('document_discount_value', $editInvoice?->document_discount_value ?? 0) }}" type="number" min="0" step="0.01" class="h-11 w-full min-w-0 rounded-l-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900">
                                <button type="button" data-invoice-doc-discount-toggle class="h-11 shrink-0 rounded-r-lg border border-l-0 border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-600 transition hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-slate-300" title="Basculer remise fixe / pourcentage">{{ $docDiscountPercentage ? '%' : 'DH' }}</button>
                            </div>
                        </label>
                    ...
                    <input type="hidden" name="document_discount_type" data-invoice-doc-discount-type value="{{ $docDiscountPercentage ? 'percentage' : 'fixed' }}">
                    <div class="grid gap-4 lg:grid-cols-3">
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Note interne</span><textarea name="internal_note" class="min-h-20 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Visible en interne uniquement...">{{ old('internal_note', $editInvoice?->internal_note) }}</textarea></label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Conditions</span><textarea name="terms" class="min-h-20 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Conditions de vente, mentions légales...">{{ old('terms', $editInvoice?->terms) }}</textarea></label>
                        <label class="space-y-1.5"><span class="text-xs font-semibold uppercase text-slate-500">Pied de page</span><textarea name="footer" class="min-h-20 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Message de bas de facture, coordonnées bancaires...">{{ old('footer', $editInvoice?->footer) }}</textarea></label>
                    </div>
                </form>
        ...
        <section class="mt-6 grid gap-6 {{ $settingsSection === 'company' ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : '' }}">
            ...
            <div class="space-y-6">
                        ...
                        <form action="{{ route('settings.pos.update') }}" method="POST" class="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/40">
                            ...
                            <div class="border-t border-slate-200 pt-4 dark:border-white/10">
                                ...
                                <div class="mt-3 grid gap-3 md:grid-cols-4">
                                    ...
                                    <label class="space-y-1.5">
                                        <span class="text-xs font-semibold uppercase text-slate-500">Activité principale</span>
                                        <select name="business_activity" required class="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm dark:border-white/10 dark:bg-slate-900">
                                            @foreach ($activityOptions as $key => $label)
                                                <option value="{{ $key }}" @selected(old('business_activity', $businessActivity) === $key)>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                </div>
                        ...
                        <section class="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                            ...
                            <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                ...
                                <label class="space-y-1.5 md:col-span-2 xl:col-span-4">
                                    <span class="text-xs font-semibold uppercase text-slate-500">Pied de ticket POS (application mobile)</span>
                                    <textarea name="receipt_footer" rows="2" placeholder="Ex : Merci de votre visite · Échanges sous 7 jours" class="min-h-16 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-white/10 dark:bg-slate-900">{{ old('receipt_footer', data_get($tenant->settings, 'receipt_footer')) }}</textarea>
                                    <span class="text-xs text-slate-400">Ce texte s'affiche en bas des tickets imprimés depuis l'application caisse.</span>
                                </label>
                                                    ...
                                                    <div class="px-6 py-5">
                                                        ...
                                                        <div class="space-y-4">
                                                            <label class="block space-y-2">
                                                                <span class="text-xs font-semibold uppercase tracking-wider text-slate-500">{{ $tr('Code PIN (4 chiffres)') }}</span>
                                                                <div class="relative">
                                                                    ...
                                                                    <input name="pin" type="password" inputmode="numeric" pattern="[0-9]{4}" minlength="4" maxlength="4" autocomplete="off" class="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-center text-lg font-bold tracking-[0.3em] outline-none transition placeholder:font-normal placeholder:tracking-normal focus:border-brand focus:ring-2 focus:ring-brand/15 dark:border-white/10 dark:bg-slate-900" placeholder="••••">
                                                                </div>
                                                            </label>
                                                            <label class="block space-y-2">
                                                                ...
                                                                <div class="relative">
                                                                    ...
                                                                    <input name="pin_confirmation" type="password" inputmode="numeric" pattern="[0-9]{4}" minlength="4" maxlength="4" autocomplete="off" class="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-center text-lg font-bold tracking-[0.3em] outline-none transition placeholder:font-normal placeholder:tracking-normal focus:border-brand focus:ring-2 focus:ring-brand/15 dark:border-white/10 dark:bg-slate-900" placeholder="••••">
                                                                </div>
                                                            </label>
                                                            @if ($user->pin_hash)
                ...
                <article class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
                    ...
                    <form action="{{ route('settings.roles.store') }}" method="POST" class="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">@csrf<div class="grid gap-3 lg:grid-cols-[1fr_1fr_auto]"><input name="name" required class="h-11 rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="Nom du rôle"><input name="key" required class="h-11 rounded-lg border border-slate-200 px-3 text-sm dark:border-white/10 dark:bg-slate-900" placeholder="clé: manager_stock"><button class="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">Créer rôle</button></div><div class="mt-3 space-y-3">@foreach ($permissionGroups as $groupKey => $group)<fieldset class="rounded-xl border border-slate-200 p-3 dark:border-white/10"><legend class="flex items-center gap-2 px-1 text-xs font-bold uppercase tracking-wide text-slate-500">{{ $group['label'] }}<button type="button" data-perm-group="{{ $groupKey }}" class="rounded border border-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-brand dark:border-white/10">Tout</button></legend><div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">@foreach ($group['permissions'] as $key => $label)<label class="flex items-start gap-2 text-sm"><input name="permissions[]" value="{{ $key }}" type="checkbox" data-perm-in="{{ $groupKey }}" @checked(in_array($key, old("permissions", []), true)) class="mt-0.5 size-4 accent-[var(--brand-primary)]"> <span>{{ $label }}</span></label>@endforeach</div></fieldset>@endforeach</div></form>
    ...
    @if (($settingsSection ?? null) === 'roles' && request('edit_role'))
        ...
        <script>
            // "Tout" ticks or unticks a whole section. With sixty-five
            // permissions across thirteen groups, building a role one
            // checkbox at a time is how a section gets missed.
            document.querySelectorAll('[data-perm-group]').forEach((button) => {
                button.addEventListener('click', () => {
                    const form = button.closest('form');
                    if (!form) return;
                    const boxes = form.querySelectorAll(
                        `[data-perm-in="${button.dataset.permGroup}"]`,
                    );
                    // Mirror what the section already is: if anything is
                    // unticked the button fills the section, otherwise it
                    // clears it.
                    const fill = Array.from(boxes).some((box) => !box.checked);
                    boxes.forEach((box) => { box.checked = fill; });
                });
            });
        </script>
    @endif
